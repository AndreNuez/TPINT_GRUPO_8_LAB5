...
public class ControladorABMTurnos {
	...
	public ModelAndView buscarPacientePorDni(@RequestParam("dni") String dni, HttpSession session) {
	    ...
	    // Mensaje de depuraci�n: Imprimir el DNI recibido
	    if (dni == null || dni.isEmpty()) {
	        System.out.println("DNI ACA LLEGA");
	        // Puedes manejar esto seg�n tus necesidades (por ejemplo, redirigir o mostrar un mensaje de error)
	    } else {
	        // Obtener el paciente
	        Paciente paciente = pacienteNg.obtenerPacientePorDNI(dni);

	        // Mensaje de depuraci�n: Verificar si el paciente fue encontrado
	        if (paciente != null) {
